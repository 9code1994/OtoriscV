...
//!
//! Implements RV64F and RV64D instruction semantics
...
impl Cpu64 {
    ...
    pub fn execute_load_fp(&mut self, inst: u32, d: &DecodedInst, bus: &mut impl Bus) -> Result<(), Trap64> {
        ...
        let paddr = match self.mmu.translate(vaddr, AccessType::Load, priv_level, bus, satp, mstatus) {
            Ok(pa) => pa,
            Err(cause) => {
                return Err(Trap64::from_cause(cause, vaddr));
            }
        };
        ...
        match d.funct3 {
            FUNCT3_FLW => {
                ...
                if vaddr & 3 != 0 {
                    return Err(Trap64::LoadAddressMisaligned(vaddr));
                }
    ...
    pub fn execute_store_fp(&mut self, inst: u32, d: &DecodedInst, bus: &mut impl Bus) -> Result<(), Trap64> {
        ...
        match d.funct3 {
            ...
            FUNCT3_FLD => {
                ...
                bus.write32(addr.wrapping_add(4), (value >> 32) as u32);
            }
            _ => return Err(Trap64::IllegalInstruction(inst as u64)),
        }
        ...
        match d.funct7 {
            ...
            FUNCT7_FCVT_W_S => {
                ...
                let (result, flags) = if d.rs2 == 0 {
                    ...
                    // FCVT.WU.S (unsigned)
                    let (v, f) = fpu::f32_to_u32(rs1, rm);
                    (v as u64, f)
                } else if d.rs2 == 2 {
                    // FCVT.L.S (signed)
                    let (v, f) = fpu::f32_to_i64(rs1, rm);
                    (v as u64, f)
                } else if d.rs2 == 3 {
                    // FCVT.LU.S (unsigned)
                    let (v, f) = fpu::f32_to_u64(rs1, rm);
                    (v, f)
                } else {
                    return Err(Trap64::IllegalInstruction(inst as u64));
                };
            ...
            FUNCT7_FCVT_S_W => {
                ...
                let (result, flags) = if d.rs2 == 0 {
                    ...
                    // FCVT.S.WU (unsigned)
                    fpu::u32_to_f32(rs1 as u32, rm)
                } else if d.rs2 == 2 {
                    // FCVT.S.L (signed)
                    fpu::i64_to_f32(rs1 as i64, rm)
                } else if d.rs2 == 3 {
                    // FCVT.S.LU (unsigned)
                    fpu::u64_to_f32(rs1, rm)
                } else {
                    return Err(Trap64::IllegalInstruction(inst as u64));
                };
                self.fpu.fflags.merge(flags);
            ...
            FUNCT7_FMV_W_X => {
                if d.funct3 == 0 && d.rs2 == 0 {
                    ...
                    self.fpu.write_f32(d.rd, value as u32);
                } else {
                    return Err(Trap64::IllegalInstruction(inst as u64));
                }
            }
            ...
            FUNCT7_FCVT_W_D => {
                ...
                if d.rs2 == 0 {
                    ...
                    self.write_reg(d.rd, v as i64 as u64);
                } else if d.rs2 == 1 {
                    let (v, f) = fpu::f64_to_u32(rs1, rm);
                    self.fpu.fflags.merge(f);
                    self.write_reg(d.rd, v as u64);
                } else if d.rs2 == 2 {
                    let (v, f) = fpu::f64_to_i64(rs1, rm);
                    self.fpu.fflags.merge(f);
                    self.write_reg(d.rd, v as u64);
                } else if d.rs2 == 3 {
                    let (v, f) = fpu::f64_to_u64(rs1, rm);
                    self.fpu.fflags.merge(f);
            ...
            FUNCT7_FCVT_D_W => {
                ...
                let rs1 = self.read_reg(d.rs1);
                if d.rs2 == 0 {
                    let result = fpu::i32_to_f64(rs1 as i32);
                    self.fpu.write_f64(d.rd, result);
                } else if d.rs2 == 1 {
                    let result = fpu::u32_to_f64(rs1 as u32);
                    self.fpu.write_f64(d.rd, result);
                } else if d.rs2 == 2 {
                    let (result, flags) = fpu::i64_to_f64(rs1 as i64);
                    self.fpu.fflags.merge(flags);
                    self.fpu.write_f64(d.rd, result);
                } else if d.rs2 == 3 {
                    let (result, flags) = fpu::u64_to_f64(rs1);
                    self.fpu.fflags.merge(flags);
                    self.fpu.write_f64(d.rd, result);
                } else {
                    return Err(Trap64::IllegalInstruction(inst as u64));
